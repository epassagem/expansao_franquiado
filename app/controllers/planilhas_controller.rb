class PlanilhasController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @planilha.save
        ...
        format.json { render action: 'show', status: :created, location: @planilha }
      else
  ...
  def destroy
    @planilha.destroy
    respond_to do |format|
      format.html { redirect_to planilhas_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def planilha_params
      params.require(:planilha).permit(:estatus, :interessado_id)
    end
end
